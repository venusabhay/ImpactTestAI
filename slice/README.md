...
**Current status: `ADAPT_ARCHITECTURE_DISCOVERY` v9 accepted as the baseline on `main` (tagged `architecture-discovery-v9-baseline`); `ARTIFACT_HISTORY` milestone in progress on `feature/artifact-history`.** v9 (`PASS`) fixed the last discovery gap found in held-out testing — route-call detection was strictly line-based, missing any call formatted with its path argument on a following line — and was merged to `main` once accepted. Full v9 report: [`reports/adapt-architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v9_final_report.md`](reports/adapt-architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v9_final_report.md) (prior rounds: [v8](reports/adapt-architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v8_final_report.md), [v7](reports/adapt-architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v7_final_report.md), [v6](reports/adapt-architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v6_final_report.md), [v5](reports/adapt-architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_report.md)).

**`ARTIFACT_HISTORY` (in progress):** every execution now gets a unique `run_id` and preserves `report.md`/`audit.json`/`metadata.json` together, immutably, under `artifacts/<organization>/<repository>/<run_id>/` — never overwritten, never deduplicated even for identical repo/commit/tool-version inputs (needed to detect future nondeterminism). No change to the risk/decision policy, architecture discovery, `design8.md`, or `design9.md`. Design: [`ARTIFACT_HISTORY_DESIGN.md`](ARTIFACT_HISTORY_DESIGN.md).
