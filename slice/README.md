...
**Current status: `ADAPT_ARCHITECTURE_DISCOVERY` v9 and `ARTIFACT_HISTORY` are both accepted and merged to `main`** (tagged `architecture-discovery-v9-baseline` and `artifact-history-baseline` respectively). v9 (`PASS`) fixed the last discovery gap found in held-out testing — route-call detection was strictly line-based, missing any call formatted with its path argument on a following line. Full v9 report: [`../pilot/investigations/architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v9_final_report.md`](../pilot/investigations/architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v9_final_report.md) (prior rounds: [v8](../pilot/investigations/architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v8_final_report.md), [v7](../pilot/investigations/architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v7_final_report.md), [v6](../pilot/investigations/architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_v6_final_report.md), [v5](../pilot/investigations/architecture-discovery/ADAPT_ARCHITECTURE_DISCOVERY_report.md)).

**`ARTIFACT_HISTORY` (accepted):** every execution now gets a unique `run_id` and preserves `report.md`/`audit.json`/`metadata.json` together, immutably, under `artifacts/<organization>/<repository>/<run_id>/` — never overwritten, never deduplicated even for identical repo/commit/tool-version inputs (needed to detect future nondeterminism). No change to the risk/decision policy, architecture discovery, `design8.md`, or `design9.md`. Design: [`../docs/decisions/ARTIFACT_HISTORY_DESIGN.md`](../docs/decisions/ARTIFACT_HISTORY_DESIGN.md); full report: [`../pilot/reports/ARTIFACT_HISTORY_final_report.md`](../pilot/reports/ARTIFACT_HISTORY_final_report.md).
...
**Stage 2C — cross-repository pilot (accepted):** ran the exact Stage 2B implementation, unmodified, against `user-management-app` — a repository with no `services/<name>/` layout, `express.Router()` instead of `app.METHOD()`, in-process auth middleware, and no CI. Three real changes were tested; impact/evidence/validation were empty in all three, and a genuine cross-component contract break scored identically to a trivial UI tweak. Full findings, ground-truth verification, and per-dimension usefulness scores: [`../pilot/cases/user-management-app-pilot/user-management-app-pilot-report.md`](../pilot/cases/user-management-app-pilot/user-management-app-pilot-report.md). The three test changes are preserved as reusable fixtures: [`../pilot/cases/user-management-app-pilot/fixtures/`](../pilot/cases/user-management-app-pilot/fixtures/), with acceptance criteria in [`RERUN_ACCEPTANCE_CRITERIA.md`](../pilot/cases/user-management-app-pilot/RERUN_ACCEPTANCE_CRITERIA.md). Recommendation was `ADAPT_ARCHITECTURE_DISCOVERY` — this recommendation was carried out across five rounds (v5 through v9) and reached `PASS`; see the current-status banner at the top of this file and the linked final reports for that full history.
...
- [`../pilot/cases/verify-cache-change-report.md`](../pilot/cases/verify-cache-change-report.md) — the actual output for the demonstration change (see below).
- [`../pilot/cases/verify-cache-change-report.audit.json`](../pilot/cases/verify-cache-change-report.audit.json) — the structured record backing that report.
- [`../pilot/cases/vertical-slice-package.md`](../pilot/cases/vertical-slice-package.md) — the business-readable package (evidence available vs. missing, limitations, next-value opportunities) requested alongside the report.
...
**Validation timeout:** each selected validation command gets `--validation-timeout-seconds` (default **180**, unchanged from prior behavior) to complete. A command that exceeds it is reported `INCONCLUSIVE` (`INFRASTRUCTURE (timeout)`), never guessed at as a pass or a fail — and an `INCONCLUSIVE` outcome still always produces `ESCALATE`, exactly as before. Raising this value only gives a genuinely slow-but-healthy validation command more wall-clock time to actually finish; it does not change what counts as a pass, a fail, or how risk is assessed. The timeout actually used is recorded on every outcome, in both `report.md` and `audit.json`, so a report is self-describing without reading source code. See [`../docs/decisions/VALIDATION_TIMEOUT_PROPOSAL.md`](../docs/decisions/VALIDATION_TIMEOUT_PROPOSAL.md) and [`../docs/decisions/VALIDATION_TIMEOUT_DISPOSITION.md`](../docs/decisions/VALIDATION_TIMEOUT_DISPOSITION.md) for the design comparison and implementation review — including the real diagnostic (an external repository whose healthy test suite legitimately took longer than the old fixed limit) that motivated making this configurable. That diagnostic is evidence for why the timeout needed to be configurable; it is not a special case in the analyzer, which has no repository-specific behavior anywhere.
